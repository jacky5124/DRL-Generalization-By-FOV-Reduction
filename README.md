# Brief Introduction
This repository contains the project Deep Reinforcement Learning Generalization by Field Of View Reduction. This project proposes a data augmentation technique that improves deep reinforcement learning generalization by simply reducing field of view in role-playing environments whose observations have agent-controlled figures appeared at center of screen. Experiments show that FOV reduction could improve DRL generalization by around 15% in [OpenAI ProcGen](https://github.com/openai/procgen) games CoinRun and CaveFlyer. The project paper can be found at project.pdf.

The code used to run experiments is written directly upon [the Amazon SageMaker Reinforcement Learning starter kit](https://github.com/aws-samples/sagemaker-rl-procgen-ray) (this repository duplicates it) which was originally for [NeurIPS 2020 - Procgen competition](https://www.aicrowd.com/challenges/neurips-2020-procgen-competition). The code is located in the Jupyter notebook project.ipynb inside the sagemaker folder as well as in the Python source files including train-project.py inside sagemaker/source folder, {cutout_env_wrapper.py, zoom_in_wrapper.py} inside sagemaker/source/custom/envs folder, and my_dqn_nature_cnn.py inside sagemaker/source/custom/models folder. Note that in order to run the experiments, you would need to follow the instructions below to prepare the solution template on Amazon SageMaker, upload the project source code to the template, and run the notebook. The sagemaker/results folder just contains my experiment result files, and project.ipynb contains graph plotting code to visualize them.
